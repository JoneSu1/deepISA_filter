"""
Standalone CLI for filters.

Usage:
    python -m deepISA.filters.cli.run_filter \\
...
_src_root = Path(__file__).resolve().parents[3]  # cli → filters → deepISA → src
if str(_src_root) not in sys.path:
    sys.path.insert(0, str(_src_root))

from deepISA.filters.cli.common_args import add_common_args
from deepISA.filters.pipeline import run_pipeline
