...
def setup_mocks(monkeypatch):
    ...
    module_path = "deepISA.explore.tf_pair_ppi"
...
def test_plot_ppi_enrichment_logic(sample_df, setup_mocks):
    ...
    with patch("seaborn.lineplot") as mock_lp:
        plot_ppi_enrichment(sample_df.copy(), rank_by="coop_score", outpath=None)
...
def test_plot_dna_mediated_ppi_execution(sample_df, setup_mocks, tmp_path):
    """
    ...
    out = tmp_path / "dna_mediated_ppi.png"
    plot_dna_mediated_ppi(sample_df.copy(), rank_by="coop_score", outpath=str(out))
    assert out.exists()



def test_plot_cofactor_recruitment_calls(sample_df, setup_mocks):
    with patch("deepISA.explore.tf_pair_ppi.plot_box_strip_statistics") as mock_box:
